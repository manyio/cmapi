...
def setup_hdfs_ha():
    ...
    try:
        ...
        if len(hdfs.get_roles_by_type("NAMENODE")) != 2:
            ...
            if hdfs.get_roles_by_type("NAMENODE")[0].hostRef.hostId == \
                    hdfs.get_roles_by_type("SECONDARYNAMENODE")[0].hostRef.hostId:
                standby_host_id = random.choice([x for x in jn if x not in
                                                 [hdfs.get_roles_by_type("NAMENODE")[0].hostRef.hostId,
                                                  hdfs.get_roles_by_type("SECONDARYNAMENODE")[0].hostRef.hostId]])
            else:
                standby_host_id = random.choice([x for x in jn if x !=
                                                 hdfs.get_roles_by_type("NAMENODE")[0].hostRef.hostId])
...
class ManagementActions:
    ...
    def licensed(cls):
        ...
        try:
            ...
            return not "Express" in err.message
    ...
    def hostname_resolves(hostname):
        ...
        try:
            if socket.gethostbyname(hostname) == '0.0.0.0':
                print "Error [{'host': '%s', 'fqdn': '%s'}]" % (socket.gethostbyname(hostname), socket.getfqdn(hostname))
                return False
            else:
                print "Success [{'host': '%s', 'fqdn': '%s'}]" % (socket.gethostbyname(hostname), socket.getfqdn(hostname))
                return True
        except socket.error:
